...
export const DynamicSelectForm: React.FC<DynamicSelectFormProps> = ({ type }) => {
  ...
  const key = type === TypeStructure.org ? 'organizationalFields' : 'geographicalFields'
  const keyType = type === TypeStructure.org ? 'organizationalElements' : 'geographicalElements'

  const elemTraversed = modelState?.[key] ?? []
  const hideForm = elemTraversed.length > 0 && type === TypeStructure.geo
    ...
    <Box>
      {configData && !hideForm && (
        <FormMain
          ...
          ListAux={ListAux ?? []}
          type={type}
          disabled={!modelState[keyType][0].hasMany && elemTraversed.length > 0}
        />
      )}
