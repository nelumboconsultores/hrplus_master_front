...
export const useColumns = ({ status, visualize, delete: deleteModel, edit }: Params) => {
  const { loadingStatus } = useContext(WorkPositionsContext)
  const { t } = useTranslation()
  const columns: GridColDef<Model>[] = [
    {
      field: 'createdAt',
      headerName: t(`instancesWorkPositions.view.columns.registerDate`),
      flex: 1,
      sortable: true,
    ...
    {
      field: 'code',
      headerName: t(`instancesWorkPositions.view.columns.code`),
      flex: 1,
      sortable: true,
    },
    {
      field: 'denomination',
      headerName: t(`instancesWorkPositions.view.columns.post`),
      flex: 1,
      sortable: true,
    },
    {
      field: 'status',
      sortable: true,
      headerName: t(`instancesWorkPositions.view.columns.status`),
      renderCell: ({ row }) => (
        <Typography sx={{ fontSize: '14px', color: '#828282' }}>
          {t(`instancesWorkPositions.view.status.${row.status.name.toLowerCase()}`)}
        </Typography>
      ),
    },
    {
